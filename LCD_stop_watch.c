        ...
        while(min!=60)
        {
            ...
            while(dly--)
            {
                if(flag==1 && ((IOPIN0>>SW)&1)==0)
                {
                    ...
                    break;          // break out of delay loop
                }
            }

            // if reset happened ? skip LCD update and exit stopwatch loop
            if(flag==0)
                break;
