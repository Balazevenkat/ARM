...
//s32 hour, min, sec, date, month, year, day;
 s32 hour = 3, min = 0, sec = 0;
   s32 date = 30, month = 11, year = 2025,day=1;
s32 enhr=2,exhr=8;
#define ENTRY 0
#define INER 1
...
void change_time()//RTC EDIT
{
    u32 d1, d2,k;
    u32 data;
    StrLCD("SET_TIME 24 FORMAT");
	  delay_ms(1200);  
	CmdLCD(0x01);
    // ----------- HOUR INPUT -----------
    do {
...
void Entry_time()
{
    ...
    CmdLCD(0x01);
    status = check_entry_hours();   // your existing check

    if(status == 1)
    {
        check_password();           // LOGIN FIRST

        if(flag == 0)               // login FAILED
        {
            CmdLCD(0x01);
            StrLCD("Login Failed");
            CmdLCD(0xC0);
        ...
        enhr = data;   // store entry hour

        CmdLCD(0x01);
        StrLCD("Entry Hour Set");
        delay_ms(1200);
        ...
        exhr = data;   // <<< GLOBAL EXIT HOUR UPDATE

        CmdLCD(0x01);
        StrLCD("Exit Hour Set");
        CmdLCD(0xC0);
        StrLCD("Press 'B'");
        delay_ms(1500);
        CmdLCD(0x01);
